    ...
    func showAlert(animated: Bool = true) {
        if self.uncertainDeliveryAlert == nil {
            let alert = UIAlertController(
                ...
                message: String(format: NSLocalizedString("%1$@ stopped communicating with your insulin pump during a critical time. The app will continue trying to reach your pump. If it succeeds this window will disappear.\n\nIf this window remains unchanged after several minutes, attempt to reset Bluetooth and/or RileyLink.\n\nInsulin delivery information cannot be updated and no automation can continue until this is resolved.\n\nTap on the button below to learn more about other options.", comment: "Message for alert shown when delivery status is uncertain. (1: app name)"), Bundle.main.bundleDisplayName),
